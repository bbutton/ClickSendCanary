...
class TestStorage(unittest.TestCase):
    ...
    def test_store_messages_to_s3(self, mock_boto_client):
        first_message_epoch = 1740774645.0  # ✅ Fixed epoch timestamp (UTC)
        timestamp = convert_from_epoch(first_message_epoch)  # ✅ Convert to datetime

        mock_s3 = MagicMock()
        mock_boto_client.return_value = mock_s3

        expected_key = f"sms-logs/year={timestamp.strftime('%Y')}/month={timestamp.strftime('%m')}/day={timestamp.strftime('%d')}/messages-{timestamp.strftime('%H-%M-%S')}.json"

        messages = [{"message_id": "123", "status": "delivered", "date": first_message_epoch}]

        store_messages(messages)

        expected_body = json.dumps(messages, indent=2)
...
class TestStorageFileNaming(unittest.TestCase):
    ...
    def test_s3_key_format(self):
        ...
        expected_key = "year=2025/month=02/day=28/messages-14-30-45.json"
